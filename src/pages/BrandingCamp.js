...
const BrandingCamp = () => {
  ...
  useEffect(() => {
      const fetchImages = async () => {
        let result = await store.ref().child('brandC').listAll();
  ...
  const handleChange = (event) => {
    ...
    function upload(){
      ...
      const uploadTask=store.ref(`brandC/${name}`).put(imgUp);
      uploadTask.on(
          "state_changed",
          snapshot=>{
