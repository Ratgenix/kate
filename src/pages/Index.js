...
const Index = () => {
    ...
    useEffect(() => {
        const fetchImages = async () => {
          let result = await store.ref().child('images').listAll();
    ...
    const handleChange = (event) => {
      ...
      function upload(){
        ...
        const uploadTask=store.ref(`images/${name}`).put(imgUp);
        uploadTask.on(
            "state_changed",
            snapshot=>{
    ...
    //let url="https://firebasestorage.googleapis.com/v0/b/art-track.appspot.com/o/images%2Fu1nffhbfdjsa%2FN11vSOYorUM2%2FImageName.JPG?"
    //console.log(url.split(RegExp('(%2F)..*(%2F)'))[0].split(".")[0]);

/*
    const desertRef = ref(store, 'images/funnel.webp');
    new ->let imageRef = storage.refFromURL(URL);
    new -> imageRef.delete()
    */
// Delete the file
function delItem(imageName){
let newName="images/"+imageName
